...
import { InstallContext, ManualClient, McpClient } from './types.js';
...
function appSupport(ctx: InstallContext): string {
...
function xdgConfig(ctx: InstallContext): string {
  if (ctx.platform === 'win32') {
    return process.env.APPDATA || path.join(ctx.home, 'AppData', 'Roaming');
  }
  return process.env.XDG_CONFIG_HOME || path.join(ctx.home, '.config');
...
function vscodeUser(ctx: InstallContext): string {
  return path.join(appSupport(ctx), 'Code', 'User');
}

function home(ctx: InstallContext, ...sub: string[]): string {
  return path.join(ctx.home, ...sub);
}

function claudeDesktopConfig(ctx: InstallContext): string {
  if (ctx.platform === 'darwin') {
...
function zedSettings(ctx: InstallContext): string {
  if (ctx.platform === 'win32') {
...
function gooseConfig(ctx: InstallContext): string {
  if (ctx.platform === 'win32') {
