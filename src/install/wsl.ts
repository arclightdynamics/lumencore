...
export function isWsl(): boolean {
  if (process.env.LUMENCORE_FORCE_WSL === '1') return true; // test/override hook
...
export function wslDistro(): string {
 ...
 */
export function windowsHomes(): string[] {
  ...
  for (const name of fs.readdirSync(usersDir)) {
    ...
    try {
      if (fs.statSync(dir).isDirectory() && fs.existsSync(path.join(dir, 'AppData'))) out.push(dir);
    } catch {
 ...
 */
export function wslServerEntry(distro: string): ServerEntry {
