 ...
 */
export const INSTRUCTION_TARGETS: Record<string, { file: string; label: string }> = {
 ...
 */
export function writeInstructionBlock(projectDir: string, relFile: string): InstructionResult {
  ...
  if (exists) {
    ...
    if (next === cur) return { file, action: 'unchanged' };
  } else {
    next = `${block}\n`;
  }
...
export function writeInstructions(projectDir: string, ids: string[] = DEFAULT_INSTRUCTION_IDS): InstructionResult[] {
  const out: InstructionResult[] = [];
  for (const id of ids) {
    const target = INSTRUCTION_TARGETS[id];
    if (target) out.push(writeInstructionBlock(projectDir, target.file));
  }
  return out;
}
