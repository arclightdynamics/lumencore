...
function hls(h: number, l: number, s: number): [number, number, number] {
...
const SCOPE_CHOICES: Array<{ value: SplashScope; title: string; hint: string }> = [
...
export function animatedInstall(
  items: SplashItem[],
  opts: { dryRun: boolean; askScope?: boolean },
  write: (index: number) => string
): Promise<SplashResult> {
  return new Promise((resolve) => {
    const out = process.stdout;
    const sel = items.map(() => true);
    let cur = 0;
    let scopeSel = 0;
    let phase: 'scope' | 'select' | 'install' | 'done' = opts.askScope ? 'scope' : 'select';
    let order: number[] = [];
    let writePtr = 0;
    let lastWrite = 0;
    let doneAt = 0;
    ...
    function onKey(d: Buffer) {
      const s = d.toString();
      if (s === '\x03' || s === 'q' || s === 'Q' || s === '\x1b') { cancelled = phase === 'scope' || phase === 'select'; return finish(); }
      if (phase === 'scope') {
        const m = SCOPE_CHOICES.length;
        if (s === '\x1b[A' || s === 'k') scopeSel = (scopeSel - 1 + m) % m;
        else if (s === '\x1b[B' || s === 'j') scopeSel = (scopeSel + 1) % m;
        else if (s === '\r' || s === '\n') phase = 'select';
        return;
      }
      if (phase !== 'select') return;
      const n = items.length;
      if (s === '\x1b[A' || s === 'k') cur = (cur - 1 + n) % n;
      else if (s === '\x1b[B' || s === 'j') cur = (cur + 1) % n;
      else if (s === ' ') sel[cur] = !sel[cur];
      else if (s === 'a' || s === 'A') { const all = sel.every(Boolean); sel.fill(!all); }
      else if (s === '\r' || s === '\n') {
        order = items.map((_, i) => i).filter((i) => sel[i]);
        if (order.length === 0) return;
        phase = 'install'; writePtr = 0; lastWrite = Date.now();
      }
    }

    // ── overlay builders ─────────────────────────────────────────────
    function place(map: Map<string, Cell>, y: number, x: number, ch: string, color: string) {
      if (y >= 1 && y <= rows() && x >= 1 && x <= cols()) map.set(`${y},${x}`, { ch, color });
    }
    function text(map: Map<string, Cell>, y: number, x: number, str: string, color: string) {
      for (let i = 0; i < str.length; i++) place(map, y, x + i, str[i]!, color);
    }

    function buildOverlay(): Map<string, Cell> {
    ...
    function render() {
      ...
      for (let y = 1; y <= R; y++) {
        ...
        for (let x = 1; x <= C; x++) {
          ...
          if (o) {
            if (o.color !== lastColor) { buf += o.color; lastColor = o.color; }
            buf += o.ch;
            continue;
          }
          // cyan lumen wave
      ...
      if (phase === 'install') {
        if (Date.now() - lastWrite > 200) {
          if (writePtr < order.length) {
            try { summary.push(write(order[writePtr]!)); } catch (e) { summary.push(`  ✗  ${items[order[writePtr]!]!.label}: ${(e as Error).message}`); }
            writePtr++;
            lastWrite = Date.now();
          } else { phase = 'done'; doneAt = Date.now(); }
        }
