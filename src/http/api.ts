 ...
 * token is configured.
 */
export function createApiServer(token?: string): http.Server {
  return http.createServer((req, res) => {
    ...
    handle(req, res, url, route, method).catch((err) =>
...
async function handle(
  ...
  if (route === '/v1/recall' && method === 'GET') {
    ...
    if (!project) return sendJson(res, 400, { ok: false, error: 'project required' });
    const results = new SearchService(project).search({
      query: url.searchParams.get('q') ?? undefined,
      category: (url.searchParams.get('category') as MemoryCategory) ?? undefined,
      scope: (url.searchParams.get('scope') as MemoryScope) ?? 'project',
      limit: url.searchParams.get('limit') ? parseInt(url.searchParams.get('limit') as string, 10) : 10,
    });
  ...
  if (route === '/v1/list' && method === 'GET') {
    ...
    const memories = new MemoryService(project).list({
      category: (url.searchParams.get('category') as MemoryCategory) ?? undefined,
      scope: 'project',
      limit: url.searchParams.get('limit') ? parseInt(url.searchParams.get('limit') as string, 10) : 50,
    });
  ...
  if (route === '/v1/remember' && method === 'POST') {
    ...
    if (!b.project) return sendJson(res, 400, { ok: false, error: 'project required' });
    if (!b.title || !b.content || !b.category) {
      return sendJson(res, 400, { ok: false, error: 'category, title, content required' });
    }
    const memory = new MemoryService(b.project as string).create({
      category: b.category as MemoryCategory,
      title: b.title as string,
      content: b.content as string,
      tags: b.tags as string[] | undefined,
      importance: b.importance as number | undefined,
      scope: b.scope as MemoryScope | undefined,
      source: (b.source as string) ?? 'http-api',
    });
...
export function startHttpApi(opts: ApiOptions = {}): Promise<http.Server> {
...
export function shutdownHttpApi(server: http.Server): void {
  server.close();
  closeAllDatabases();
}
