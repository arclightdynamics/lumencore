 ...
 */
function candidateRoots(extra: string[] = []): string[] {
  const roots = new Set<string>(extra.map((p) => path.resolve(p)));
  const claudeJson = path.join(os.homedir(), '.claude.json');
  if (fs.existsSync(claudeJson)) {
    try {
      const d = JSON.parse(fs.readFileSync(claudeJson, 'utf-8')) as { projects?: Record<string, unknown> };
      for (const p of Object.keys(d.projects ?? {})) roots.add(p);
    } catch {
...
export function runBackfill(flags: BackfillFlags): void {
  const cm = getConfigManager();
  if (!cm.isConfigured()) cm.save(getDefaultConfig());
  ...
  for (const hash of dirs) {
    ...
    const missing = (
      db.prepare("SELECT COUNT(*) AS c FROM memories WHERE project_path IS NULL OR project_path = ''").get() as { c: number }
    ).c;
  ...
  if (!flags.dryRun) closeAllDatabases();
}
