...
import { InstallContext, McpClient, ScopeTarget, ServerEntry } from '../install/types.js';
...
import { ConfigMemoryScope } from '../types/index.js';
...
function pickTarget(targets: ScopeTarget[], flags: InstallFlags): ScopeTarget {
  if (flags.global) return targets.find((t) => t.scope === 'global') ?? targets[0];
  if (flags.project) return targets.find((t) => t.scope === 'project') ?? targets[0];
  return targets[0]; // recommended default
}

function windowsContexts(projectDir: string): InstallContext[] {
  return windowsHomes().map((home) => ({ projectDir, platform: 'win32' as NodeJS.Platform, home }));
}

/** Every (client, host) install action available in this environment. */
function gatherActions(ctx: InstallContext, flags: InstallFlags): Action[] {
  const actions: Action[] = [];
  const filter = (cs: McpClient[]) => (flags.client ? cs.filter((c) => c.id === flags.client) : cs);
  ...
  if (isWsl() && !flags.noWindows) {
    ...
    for (const winCtx of windowsContexts(ctx.projectDir)) {
      for (const client of filter(detectClients(winCtx))) {
        ...
        if (globals.length === 0) continue; // project-scope is meaningless for a Windows client from WSL
...
function printList(ctx: InstallContext): void {
  ...
  if (isWsl()) {
    ...
    if (winCtxs.length === 0) console.log('  (no Windows user profiles found under /mnt/c/Users)');
  ...
  console.log('\nManual / coming soon:');
  for (const m of MANUAL_CLIENTS) console.log(`  • ${m.name} — ${m.reason}`);
  console.log('');
}

/** Memory scope pinned by an explicit flag, or null when we should ask. */
function scopeFromFlags(flags: InstallFlags): ConfigMemoryScope | null {
  if (flags.shareGlobal === true) return 'project-and-global';
  if (flags.shareGlobal === false) return 'project-only';
  return null;
}
 ...
 */
async function promptMemoryScope(flags: InstallFlags): Promise<ConfigMemoryScope | null> {
  if (flags.yes || !process.stdin.isTTY) return null;
  ...
  return (res.scope as ConfigMemoryScope) ?? null;
}

/** Persist the chosen memory scope into the install-wide config (no-op on dry run). */
function applyMemoryScope(scope: ConfigMemoryScope, dryRun: boolean): void {
  if (dryRun) return;
...
export async function runInstall(flags: InstallFlags): Promise<void> {
  ...
  if (flags.music) playJingle();

  const writeOne = (a: Action): string => {
    try {
  ...
  if (animate) {
    try {
      const res = await animatedInstall(
        actions.map((a) => ({ label: a.label })),
        { dryRun: flags.dryRun, askScope: memoryScope === null },
        (i) => writeOne(actions[i])
      );
      if (res.cancelled) {
        console.log('\n  Cancelled — nothing written.\n');
        return;
      }
      if (res.scope) memoryScope = res.scope;
      if (memoryScope) applyMemoryScope(memoryScope, flags.dryRun);
      console.log('  Results:');
  ...
  if (isWsl() && !flags.noWindows) console.log(`  Environment: WSL (${wslDistro()}) — Windows clients can share this brain via wsl.exe`);
  ...
  if (memoryScope === null) memoryScope = await promptMemoryScope(flags);
  if (memoryScope) applyMemoryScope(memoryScope, flags.dryRun);
  ...
  if (interactive) {
    ...
    chosen = (res.sel as number[]).map((i) => actions[i]);
  }
