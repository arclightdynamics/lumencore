...
function formatConflicts(conflicts) {
    ...
    return (`⚠️ POSSIBLE CONFLICTS — ${conflicts.length} existing ${noun} with this one:\n\n${list}\n\n` +
        ...
        try {
            switch (name) {
                ...
                case 'recall': {
                    ...
                    memoryService.recordAccess(memories.map((m) => ({ id: m.id, scope: m.scope })));
...
`;
                ...
                case 'supersede_memory': {
                    const oldId = args?.old_id;
                    const newId = args?.new_id;
                ...
                case 'capture_turn': {
                    ...
                    if (text) {
                        ...
                        if (hints.length > 0) {
                            const lines = hints.map((h, i) => `${i + 1}. [${h.category}] ${h.text}\n   (signal: "${h.signal}", suggested importance ${h.importance})`);
                            sections.push(`Possible memories detected — review and store the durable ones with remember:\n${lines.join('\n')}`);
                        }
                    }
